...
メモ:
- **`npm run dev` の代わりに `npm run studio` で起動すると、保存するだけで自動 commit & push → 自動公開**されます（`git push` 不要）。手で確認してから公開したい時は `npm run dev`。
- このエディタは**手元で起動中だけ**動きます（本番サイトは静的のまま）。将来、スマホ/ブラウザのクラウド編集にも拡張できます。
- **作品(Work)** も `/keystatic` の「作品 (Work)」で編集できます（本文はリッチエディタ、カバー画像アップロード可）。下の 1. はコマンド/ファイルで追加する代替手段です。
